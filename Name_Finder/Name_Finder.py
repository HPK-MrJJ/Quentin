...
class Name_Finder(commands.Cog):
    ...
    async def download_and_process_xml(url):
        try:
            req = urllib.request.Request(url, headers={'User-Agent': 'kakastania'})
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    try:
                        await ctx.send(file=discord.File("available_nations.txt"))
                    finally:
    ...
    async def send_daily_message(self):
        ...
        try:
            await download_and_save_xml(f'https://www.nationstates.net/archive/nations/{date}-nations-xml.gz')
        except Exception as e:
            return
